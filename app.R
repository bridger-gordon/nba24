...
nba_pbp_2024_raw <- hoopR::load_nba_pbp(seasons = hoopR::most_recent_nba_season())
nba_box_2024 <- hoopR::load_nba_player_box(seasons = hoopR::most_recent_nba_season())
player_tibble <- nba_box_2024 %>%
  select(athlete_id, athlete_display_name) %>%
  unique()
team_tibble <- nba_box_2024 %>%
  select(team_id, team_name) %>%
  unique()
clean_PBP24 <- nba_pbp_2024_raw %>%
...
positionalSplits <- nba_box_2024 %>%
  ...
  mutate(rankPoints = order(order(pointsAllowedByPos, decreasing = T)),
         rankRebs = order(order(pointsAllowedByPos, decreasing = T)),
         rankAssists = order(order(reboundsAllowedByPos, decreasing = T)),
...
trending <- nba_box_2024 %>%
  dplyr::filter(!is.na(minutes)) %>%
  group_by(athlete_display_name) %>%
  arrange(game_date) %>%
  mutate(gamesPlayed = n()) %>%
  dplyr::filter(gamesPlayed >= 11) %>%
  mutate(MAPoints = movavg(points, 10, type = "s"),
         MARebounds = movavg(rebounds, 10, type = "s"),
         MAAssists = movavg(assists, 10, type = "s"),
         MAFGvolume = movavg(field_goals_attempted, 10, type = "s"),
         MATvolume = movavg(three_point_field_goals_attempted, 10, type = "s"),
         MATmade = movavg(three_point_field_goals_made, 10, type = "s"))  %>%
  mutate(chgMAPoints = round( MAPoints - dplyr::lag(MAPoints, 10),2),
         chgMARebounds = round( MARebounds - dplyr::lag(MARebounds, 10), 2),
         chgMAAssists = round( MAAssists - dplyr::lag(MAAssists, 10), 2),
         chgMAFGvolume = round( MAFGvolume - dplyr::lag(MAFGvolume, 10), 2),
         chgMATvolume = round( MATvolume - dplyr::lag(MATvolume, 10), 2),
         chgMATmade = round( MATmade - dplyr::lag(MATmade, 10), 2)) %>%
  mutate(player = athlete_display_name)

# leading start
PointsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MAPoints)) %>%
  head(10) %>%
  select(player, team_name, MAPoints, chgMAPoints)

ReboundsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MARebounds)) %>%
  head(10) %>%
  select(player, team_name, MARebounds, chgMARebounds)

AssistsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MAAssists)) %>%
  head(10) %>%
  select(player, team_name, MAAssists, chgMAAssists)


FGATable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MAFGvolume)) %>%
  head(10) %>%
  select(player, team_name, MAFGvolume, chgMAFGvolume)

TATable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MATvolume)) %>%
  head(10) %>%
  select(player, team_name, MATvolume, chgMATvolume)

TMTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(MATmade)) %>%
  head(10) %>%
  select(player, team_name, MATmade, chgMATmade)
# leading end

# trending up start
trendingPointsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMAPoints)) %>%
  head(10) %>%
  select(player, team_name, MAPoints, chgMAPoints)

trendingReboundsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMARebounds)) %>%
  head(10) %>%
  select(player, team_name, MARebounds, chgMARebounds)

trendingAssistsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMAAssists)) %>%
  head(10) %>%
  select(player, team_name, MAAssists, chgMAAssists)


trendingFGATable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMAFGvolume)) %>%
  head(10) %>%
  select(player, team_name, MAFGvolume, chgMAFGvolume)

trendingTATable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMATvolume)) %>%
  head(10) %>%
  select(player, team_name, MATvolume, chgMATvolume)

trendingTMTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMATmade)) %>%
  head(10) %>%
  select(player, team_name, MATmade, chgMATmade)
...
DtrendingReboundsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMARebounds)) %>%
  tail(5) %>%
...
DtrendingAssistsTable <- trending %>%
  slice(tail(row_number(), 1)) %>%
  ungroup() %>%
  arrange(desc(chgMAAssists)) %>%
  tail(5) %>%
...
teamstats <- nba_leaguedashteamstats(league_id = '00', season = year_to_season(most_recent_nba_season() - 1))$LeagueDashTeamStats
teamstats[,3:ncol(teamstats)] <- teamstats[,3:ncol(teamstats)] %>% mutate_if(is.character, as.numeric)
...
teamstatsDatatable <- teamstats %>%
  mutate(PPG = round(PTS / GP, 1),
         ...
         RPG = round(REB / GP, 1),
         ORPG = round(OREB / GP, 1),
         DRPG = round(DREB / GP, 1),
  ...
  navbarPage(tags$b("NBA Play-by-Play Data"),
             ...
             tabPanel("Team Stats",
                      mainPanel(h1("Team Stats, 2024 season"),
                                ...
                                datatable(teamstatsDatatable, options = list(paging = FALSE, lengthChange = FALSE), rownames = FALSE) %>% 
                                  ...
                                  formatStyle('TOPG', backgroundColor = styleInterval(c(13, 14, 15), c('#abdda5', '#e6f599','#fffebf', '#F7A9A8'))) %>%
                                ...
                                datatable(teamstatsAgainstDatatable, options = list(paging = FALSE, lengthChange = FALSE), rownames = FALSE) %>% 
                                  ...
                                  formatStyle('aTOPG', backgroundColor = styleInterval(c(13, 14, 15), c('#abdda5', '#e6f599','#fffebf', '#F7A9A8'))) %>%
             ...
             tabPanel("Today's Matchups",
                      ...
                      datatable(top5sToday, options = list(paging = FALSE, lengthChange = FALSE), rownames = FALSE, width = "100%")  %>% 
                        formatStyle('rankPoints', backgroundColor = styleInterval(c(1,5), c('#abdda5', '#e6f599', '#F7A9A8'))) %>% 
                        formatStyle('rankRebs', backgroundColor = styleInterval(c(1,5), c('#abdda5', '#e6f599', '#F7A9A8'))) %>% 
                        formatStyle('rankAssists', backgroundColor = styleInterval(c(1,5), c('#abdda5', '#e6f599', '#F7A9A8'))) %>% 
                        formatStyle('rankThrees', backgroundColor = styleInterval(c(1,5), c('#abdda5', '#e6f599', '#F7A9A8'))),
                      br(), br(), br(), br(), br(), br(),
             ...
             tabPanel("Dashboard",
                      ...
                      fluidRow(
                        column(4, 
                               h4("Points"),
                               datatable(PointsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Rebounds"),
                               datatable(ReboundsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Assists"),
                               datatable(AssistsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
                      ),
                      br(), br(),
                      fluidRow(
                        column(4, 
                               h4("FG Attempted"),
                               datatable(FGATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Threes Attempted"),
                               datatable(TATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Threes Made"),
                               datatable(TMTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
                      ),
                      br(),h4("##"),br(),
                      h3("Players Trending Up"),
                      fluidRow(
                        column(4, 
                               h4("Points"),
                               datatable(trendingPointsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Rebounds"),
                               datatable(trendingReboundsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Assists"),
                               datatable(trendingAssistsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
                      ),
                      br(), br(),
                      fluidRow(
                        column(4, 
                               h4("FG Attempted"),
                               datatable(trendingFGATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Threes Attempted"),
                               datatable(trendingTATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Threes Made"),
                               datatable(trendingTMTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
                      ),
                      br(),h4("##"),br(),br(),
                      h3("Players Trending Down"),
                      fluidRow(
                        column(4, 
                               h4("Points"),
                               datatable(DtrendingPointsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Rebounds"),
                               datatable(DtrendingReboundsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Assists"),
                               datatable(DtrendingAssistsTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
                      ),
                      br(), br(),
                      fluidRow(
                        column(4, 
                               h4("FG Attempted"),
                               datatable(DtrendingFGATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        column(4, 
                               h4("Threes Attempted"),
                               datatable(DtrendingTATable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%")),
                        
                        column(4, 
                               h4("Threes Made"),
                               datatable(DtrendingTMTable, options = list(dom = "t", ordering = FALSE, paging = FALSE, searching = FALSE), selection = 'none', 
                                         callback = JS("$('table.dataTable.no-footer').css('border-bottom', 'none');"), class = 'row-border', escape = FALSE, rownames = FALSE, filter = "none", width = "100%"))
...
server <- function(input, output, session) {
  ...
  output$pointsLineplot <- renderPlotly({
    ...
    playerBox <- nba_box_2024 %>%
      dplyr::filter(athlete_display_name == playerName) %>%
      dplyr::filter(!is.na(minutes)) %>%
      arrange(game_date) %>%
      mutate(movAvg = movavg(points, 10, type = "s"))
  ...
  output$twoOrThreeLineplot <- renderPlotly({
    ...
    playerName <- input$playersP
    playerBoxOneTwoThree <- nba_box_2024 %>%
      dplyr::filter(athlete_display_name == playerName) %>%
      dplyr::filter(!is.na(minutes)) %>%
  ...
  output$pointsPlayerShotChart <- renderPlot({
    ...
    playerName <- input$playersP
    playerShotsGamelog <- clean_PBP24 %>%
      dplyr::filter(athlete_display_name == playerName) %>%
  ...
  output$assistsTeamRank <- renderText({
    ...
    teamName <- input$teamsA
    realTeamname <- nba_box_2024 %>%
      dplyr::filter(team_name == teamName) %>%
      select(team_display_name) %>%
      unique() %>%
      pull()
    
    assistRank <- teamstats %>%
  ...
  output$threesShootingPerc <- renderPlotly({
    ...
    convChart <- player3Box %>%
      ...
      ggplot() +
      # game percentage
      geom_line(aes(x = game_date, y = threepercentage), color = "red", linewidth = 0.25) +
      geom_point(aes(x = game_date, y = threepercentage, size = volume), color = "red", show.legend = FALSE) + 
      # moving average
